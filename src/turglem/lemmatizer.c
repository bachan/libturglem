...
turglem turglem_attach(const void *ptr_dic_autom_nodes, const void *ptr_dic_autom_links, const void *ptr_predict_autom_nodes,
...
extern size_t turglem_build_form(turglem lem, const MAFSA_letter *s_in, size_t sz_s_in, MAFSA_letter *s_out, size_t sz_s_out, int src_paradigm, int src_form, int n_form)
{
    ...
    memcpy(out, new_prefix, new_prefix_sz * sizeof(uint8_t));
    out += new_prefix_sz;
    ...
    memcpy(out, s_in + old_prefix_sz, (sz_s_in - old_prefix_sz - old_suffix_sz) * sizeof(uint8_t));
    out += sz_s_in - old_prefix_sz - old_suffix_sz;

    memcpy(out, new_suffix, new_suffix_sz * sizeof(uint8_t));
    out += new_suffix_sz;
