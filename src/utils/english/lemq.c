...
int teng_getopt_parse(int argc, char **argv, teng_getopt_t *opts)
{
    ...
    while (-1 != (c = getopt_long(argc, argv, "npgP", options, NULL)))
    {
        switch (c)
        {
            ...
            case 'g': opts->show |= TENG_SHOW_GRAMMEM;  break;
...
void check_grammem_and_print(void *user_data, const MAFSA_letter *l, size_t sz_l)
{
    ...
    MAFSA_automaton_str_to_int_pair(l, sz_l, ENGLISH_LETTER_DELIM, form);
    ...
    if ((grm & *grm_mask) == *grm_mask)
    {
        ...
        sz_binary_dst = ENGLISH_conv_letter_to_binary_utf8(l, sz_l, binary_dst, 1024);
...
int main(int argc, char **argv)
{
    ...
    if (opts.show & TENG_SHOW_GRAMMEM)
    {
